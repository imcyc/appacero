...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    const rectPointsAmarillos2 = [
      ...
      new THREE.Vector3(1.75, 0, -0.6),
      new THREE.Vector3(2, 0, -0.6),
      new THREE.Vector3(2.25, 0, -0.6),
      new THREE.Vector3(2.6, 0, -0.6),
      new THREE.Vector3(2.6, 0, -0.45),
      new THREE.Vector3(2.3, 0, 0),
    ];

    const LineasAmarillas = Array.from({ length: 1 }, (_, i) => {
      const curve = new THREE.CatmullRomCurve3(rectPointsAmarillos2, false);
      const curvePoints = curve.getPoints(100);

      //Convertir los puntos a LineGeometry
      const positions = curvePoints.flatMap((p) => [p.x, p.y, p.z]);
      const lineGeometry = new LineGeometry();
      lineGeometry.setPositions(positions);
      ...
      const line = new Line2(lineGeometry, lineMaterial);
      line.position.y = 0.4 + i * 2;

      return line;
    });

    LineasAmarillas.forEach((line) => scene.add(line));

    //Agregar rectangulos Amarillos
    const rectPointsAmarillos3 = [
      //Linea Derecha Vertical
      new THREE.Vector3(1.1, 0, 2.3),
      new THREE.Vector3(1.45, 0, 2.6),
      ...
      new THREE.Vector3(1.6, 0, 2.6),
      new THREE.Vector3(1.6, 0, 2.45),
      new THREE.Vector3(1.3, 0, 2.1),
    ];

    const LineasAmarillas3 = Array.from({ length: 2 }, (_, i) => {
      const curve = new THREE.CatmullRomCurve3(rectPointsAmarillos3, false);
      const curvePoints = curve.getPoints(100);
      ...
      const line = new Line2(lineGeometry, lineMaterial);
      line.position.y = -1 + i * 3.2;

      return line;
    });

    LineasAmarillas3.forEach((line) => scene.add(line));
    ...
    const rectPointsAmarillos12 = [
      ...
      new THREE.Vector3(-1.25, 0, 1.6),
      new THREE.Vector3(-1.6, 0, 1.6),
      new THREE.Vector3(-1.4, 0, 1.6),
      new THREE.Vector3(-0.8, 0, 1.4),
    ];

    const LineasAmarillas12 = Array.from({ length: 2 }, (_, i) => {
      const curve = new THREE.CatmullRomCurve3(rectPointsAmarillos12, false);
      const curvePoints = curve.getPoints(100);
      ...
      const line = new Line2(lineGeometry, lineMaterial);
      line.position.y = -1.3 + i * 3.2;

      return line;
    });

    LineasAmarillas12.forEach((line) => scene.add(line));
    ...
    const rectPointsAmarillos32 = [
      ...
      new THREE.Vector3(-0.25, 0, -1.6),
      new THREE.Vector3(-0.6, 0, -1.6),
      new THREE.Vector3(-0.6, 0, -1.45),
      new THREE.Vector3(-0.3, 0, -1.1),
    ];

    const LineasAmarillas32 = Array.from({ length: 1 }, (_, i) => {
      const curve = new THREE.CatmullRomCurve3(rectPointsAmarillos32, false);
      const curvePoints = curve.getPoints(100);
      ...
      const line = new Line2(lineGeometry, lineMaterial);
      line.position.y = 0.6 + i * 3.2;

      return line;
    });

    LineasAmarillas32.forEach((line) => scene.add(line));
